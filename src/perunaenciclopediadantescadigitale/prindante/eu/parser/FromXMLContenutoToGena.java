 ...
 */
public class FromXMLContenutoToGena {
	
	public OperaContenutoInfo readXMLFiles(String xmlFileDir) {
		OperaContenutoInfo operaContenuto = null;
		try{
			String files;
			File folder = new File(xmlFileDir);
			...
			for (int i = 0; i < listOfFiles.length; i++) 
			{
				if (listOfFiles[i].isFile()) 
				{
					files = listOfFiles[i].getName();
					   ...
					   FileInputStream fileInputStream = new FileInputStream(xmlFileDir+File.separator+files);
					   System.out.println("file:"+xmlFileDir+File.separator+files);
					   XMLInputFactory inputFactory = XMLInputFactory.newInstance();  
		               XMLStreamReader xmlReader = inputFactory.createXMLStreamReader(fileInputStream);
					 //  XMLEventReader xmlReader = inputFactory.createXMLEventReader(fileInputStream);
