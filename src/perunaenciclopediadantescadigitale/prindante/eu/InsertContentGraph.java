...
import perunaenciclopediadantescadigitale.prindante.eu.parser.XMLContentToJena;

/**
 * @author Daniele
 * Insert Content into Virtuoso database
 */
public class InsertContentGraph {
	/**
	 * @param args
	 * @throws Exception 
	 */	
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			configFile.load(InsertContentGraph.class.getClassLoader().getResourceAsStream("config.properties"));
		}
		catch (IOException e) {
			System.out.println("\nINSERT - Errore nel caricamento della configurazione");
		}
		String contentDir = configFile.getProperty("directoryContenuto");
		
		// INIZIALIZZA DIRECTORY XML E RDF
		File xmlDir = new File(contentDir + sep + "XML");
		File rdfDir = new File(contentDir + sep + "RDF");
		...
		for (String title: toTriplify) {
			...
			ModelJenaXMLContenuto modelXmlContent = new ModelJenaXMLContenuto(new XMLContentToJena().readXMLContentFile(xmlPath));
			
			// GENERA FILE RDF
			String rdfPath = rdfDir + sep + title + ".rdf";
			System.out.println("INSERT - Genero file RDF: " + rdfPath);
			modelXmlContent.createRDFGraphModel(rdfPath, baseURI + "/work/" + title.toLowerCase());
		}
		...
		for (String title: toInsert) {
			...
			String graphURI = baseURI + "/" + title.toLowerCase() + "/contenuto";
			virtDB.deleteGraph(graphURI);
			
			//INSERISCI NUOVO GRAFO
			String rdfPath = configFile.getProperty("directoryContenuto") + sep + "RDF" + sep + title + ".rdf";
			virtDB.insertGraph(graphURI, rdfPath);
		}
		
		// CANCELLA GRAFO OPERE ESISTENTE
		virtDB.deleteGraph(baseURI + "/opere/contenuto");
		
		// INSERISCI NUOVO GRAFO OPERE
		virtDB.insertFullGraph(baseURI + "/opere/contenuto", configFile.getProperty("directoryContenuto") + sep + "RDF");
